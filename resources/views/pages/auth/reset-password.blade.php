...
<x-layout.login-layout :title="__('page-title.reset-password')">
    ...
    <main class="login">
        <div class="h-svh grid-login px-default max-limit-width">
            <div
                ...
                <h1 class="text-2xl rg:text-3xl font-medium text-brown text-center pb-2 pt-6">{{ __('auth/reset-password.h1') }}</h1>
                <p class="paragraph text-gray-600 text-center mb-6">
                    {!! __('forms.accessibility_text')  !!}
                </p>

                {{-- FORMULAIRE --}}
                <x-pages.auth.reset-password-form
                    class="w-full"/>
            </div>
            <div class="relative max-rl:hidden rl:col-start-1 rl:col-span-7 rl:row-start-1">
                <img
                    ...
                    {{ asset('assets/img/jpg/login/side-login-600w.webp') }} 600w,
                    {{ asset('assets/img/jpg/login/side-login-800w.webp') }} 800w,
