<fieldset class="col-span-full lg:col-span-2 items-center lg:border-none!">
    ...
    <div class="flex flex-col gap-4">
        <label for="profile-picture"
            ...
            @if($this->form->avatar || ($this->form->member && $this->form->member->avatar_path && Storage::disk(config('filesystems.default'))->exists(config('avatar.original_path') . '/' . $this->form->member->avatar_path)))
        ...
        @if($this->form->avatar || ($this->form->member && $this->form->member->avatar_path && Storage::disk(config('filesystems.default'))->exists(config('avatar.original_path') . '/' . $this->form->member->avatar_path)))
            <button type="button"
                    @if($this->form->member)
                        wire:click="removeOldAvatar({{ $this->form->member->id }})"
                    @else
